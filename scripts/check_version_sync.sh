#!/usr/bin/env bash
# Fail when current release/status files drift from the canonical VERSION value.
set -euo pipefail
...
# Release scripts should derive from the shared resolver, not parse VERSION in
# their own slightly different ways.
if ! grep -Fq '.github/scripts/resolve_version.sh --base' scripts/release_build.sh; then
  note_fail "scripts/release_build.sh does not use the shared version resolver"
fi
